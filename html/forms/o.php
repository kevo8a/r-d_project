...
if (isset($_POST['id']) && isset($_POST['calificacion'])) {
    ...
    if ($calificacion === 'Aprobar') {
        ...
        if ($result_form2->num_rows > 0) {
            ...
            $id_form1 = $form2_data['id_form1']; // Asumiendo que form2 tiene una referencia a form1
            ...
            if ($result_form1->num_rows > 0) {
                ...
                $sql2 = "INSERT INTO form3 (
                        ...
                        requested_qty, requested_units, printing_system

                        ) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt2 = mysqli_prepare($conn, $sql2);
                mysqli_stmt_bind_param($stmt2, "ssissssssiss", 
                        $id_form3      , $status_form3, $id_user          , 
                        $name_user     , $created_at  , $site_user        , 
                        $name_client   , $project_name, $photocell_colors ,
                        $volumen_pedido, $sales_unit  , $sistema_impresion
                    );
